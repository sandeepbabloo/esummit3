 ...
 define( 'DBHOST_', 'localhost' );
        ...
        if(isset($_POST['type'])){
          ...
          if($type == "prof"){
            ...
            //adding to db
            $stmt=$conn->prepare("insert into reg_details (name, email, phone, startup, designation,count,is_prof ) values (?,?,?,?,?,?,?)");
            echo $conn->error;
            $stmt->bind_param("sssssii",$_POST["name"][0],$_POST["email"][0],$_POST["phone"][0],$_POST["startup"],$_POST["designation"],$quantity,$is_prof);
            $stmt->execute();
            echo $conn->error;
          ...
          elseif ($type=="stud") {
            ...
            $tax = 7;
            //adding to db
            $stmt=$conn->prepare("insert into reg_details (name, email, phone, state, city, college,count,is_prof ) values (?,?,?,?,?,?,?,?)");
            echo $conn->error;
            $stmt->bind_param("ssssssii",$_POST["name"][0],$_POST["email"][0],$_POST["phone"][0],$_POST["state"],$_POST["city"],$_POST["college"],$quantity,$is_prof);
            $stmt->execute();
            echo $conn->error;
            $insert_id= $stmt->insert_id;
            $stmt->close();
